...
export function TaskCard({
  ...
  return (
    <div className="bg-[#faf5ee] rounded-2xl p-5 border border-[#e8dfd2] shadow-[6px_6px_12px_#d4c9ba,-6px_-6px_12px_#ffffff] hover:shadow-[8px_8px_16px_#d4c9ba,-8px_-8px_16px_#ffffff] transition-all duration-200">
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap mb-2">
            <Badge
              variant="outline"
              className={`text-xs font-medium ${getTypeClasses(task.type)}`}
            >
              {task.type === "assignment" ? "📝 Assignment" : "🧪 Class Test"}
            </Badge>
            <Badge
              variant="outline"
              className={`text-xs font-medium ${getPriorityClasses(task.priority)}`}
            >
              {task.priority.charAt(0).toUpperCase() + task.priority.slice(1)}
            </Badge>
            <Badge
              variant="outline"
              className={`text-xs font-medium ${getUrgencyBadgeClasses(urgency)}`}
            >
              {getUrgencyLabel(urgency)}
            </Badge>
          </div>

          <h3 className="text-lg font-semibold text-[#3d3429] truncate">
            {task.title}
          </h3>

          <p className="text-sm text-[#8b7355] mt-1">{task.subject}</p>

          {task.description && (
            <p className="text-sm text-[#6b5b47] mt-2 line-clamp-2">
              {task.description}
            </p>
          )}
